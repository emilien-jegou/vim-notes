...
function! vnotes#notes#mediawiki#convert_note(note_text) " {{{1
  ...
  call map(blocks, 'vnotes#notes#mediawiki#convert_block(v:val)')
  let mediawiki = join(blocks, "\n\n")
  call vmisc#misc#timer#stop("notes.vim %s: Converted note to Mediawiki syntax in %s.", g:vnotes#notes#version, starttime)
  return mediawiki
endfunction

function! vnotes#notes#mediawiki#convert_block(block) " {{{1
