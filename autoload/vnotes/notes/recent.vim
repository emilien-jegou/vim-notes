...
function! vnotes#notes#recent#show(bang, title_filter) " {{{1
  call vmisc#misc#msg#info("notes.vim %s: Generating overview of recent notes ..", g:vnotes#notes#version)
  ...
  call vmisc#misc#buffer#prepare({
        \ 'name': bufname,
        \ 'path': vmisc#misc#path#merge($HOME, bufname)})
  " Filter notes by pattern (argument)?
  let notes = []
  let title_filter = '\v' . a:title_filter
  for [fname, title] in items(vnotes#notes#get_fnames_and_titles(0))
  ...
  if a:title_filter != ''
    let quote_format = vnotes#notes#unicode_enabled() ? '‘%s’' : "`%s'"
  ...
  let last_date = ''
  let list_item_format = vnotes#notes#unicode_enabled() ? ' • %s' : ' * %s'
  call sort(notes)
  call reverse(notes)
  let lines = []
  for [ftime, title] in notes
    let date = vnotes#notes#friendly_date(ftime)
    if date != last_date
      call add(lines, '')
  ...
  " And we're done!
  call vmisc#misc#timer#stop("notes.vim %s: Generated %s in %s.", g:vnotes#notes#version, bufname, starttime)
endfunction

function! vnotes#notes#recent#track() " {{{1
  let fname = expand('%:p')
  let indexfile = expand(g:notes_recentindex)
  call vmisc#misc#msg#debug("notes.vim %s: Recording '%s' as most recent note in %s ..", g:vnotes#notes#version, fname, indexfile)
  if writefile([fname], indexfile) == -1
    call vmisc#misc#msg#warn("notes.vim %s: Failed to record most recent note in %s!", g:vnotes#notes#version, indexfile)
  endif
endfunction

function! vnotes#notes#recent#edit(bang) " {{{1
  " Edit the most recently edited (not necessarily changed) note.
  let indexfile = expand(g:notes_recentindex)
  call vmisc#misc#msg#debug("notes.vim %s: Recalling most recent note from %s ..", g:vnotes#notes#version, indexfile)
  try
    ...
    call vmisc#misc#msg#warn("notes.vim %s: Failed to recall most recent note from %s: %s", g:vnotes#notes#version, indexfile, v:exception)
    return
  endtry
  call vmisc#misc#msg#info("notes.vim %s: Editing most recent note '%s' ..", g:vnotes#notes#version, fname)
