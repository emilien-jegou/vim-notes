...
function! vnotes#notes#markdown#view() " {{{1
  " Convert the current note to a Markdown document and show the converted text.
  let note_text = join(getline(1, '$'), "\n")
  let markdown_text = vnotes#notes#markdown#convert_note(note_text)
  vnew
  call setline(1, split(markdown_text, "\n"))
  setlocal filetype=markdown
endfunction

function! vnotes#notes#markdown#convert_note(note_text) " {{{1
  ...
  let blocks = vnotes#notes#parser#parse_note(a:note_text)
  call map(blocks, 'vnotes#notes#markdown#convert_block(v:val)')
  let markdown = join(blocks, "\n\n")
  call vmisc#misc#timer#stop("notes.vim %s: Converted note to Markdown in %s.", g:vnotes#notes#version, starttime)
  return markdown
endfunction

function! vnotes#notes#markdown#convert_block(block) " {{{1
  " Convert a single block produced by `vmisc#misc#notes#parser#parse_note()`
  ...
  elseif a:block.type == 'code'
    ...
    let text = vmisc#misc#str#indent(vmisc#misc#str#dedent(a:block.text), 4)
...
function! s:make_urls_explicit(text) " {{{1
  ...
  return substitute(a:text, g:vnotes#notes#url_pattern, '\= s:url_callback(submatch(0))', 'g')
endfunction
